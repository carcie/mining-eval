# Financial Model & Energy-Revenue Relationship for Cryptocurrency Mining
...
* **Power‑Usage Effectiveness (PUE)** is applied:
...
where $P_{\max} = 20 \text{ MW}$. The total energy consumed in month $m$ (with $n_m$ hours) is:
...
$$R_m = \sum_{t \in m} H_{\text{eff}}(t) \times \text{hashprice}_m \times \frac{1}{24}$$

Using daily hashprice $H_{\text{daily},m}$ expressed in USD/PH/day, monthly revenue becomes:
...
If $H$ is in TH/s, we convert ($1 \text{ PH/s} = 1000 \text{ TH/s}$), yielding:

$$R_{\text{inst}}(t) = \frac{H_{\text{TH}}(t)}{1000} \cdot HP(t) \quad \text{[USD/day]}$$

### 1.3 Revenue as a function of energy consumption

Substitute $H(t) = P(t)/J$ (where $P$ is in Watts, $J$ is in J/TH):

* $P$ in Watts = Joules per second.
* $H$ in TH/s = $P / J$ (since $1 \text{ W} = 1 \text{ J/s}$, and $1 \text{ TH/s} = 10^{12} \text{ H/s}$, where $J$ is defined as $J$ Joules per $10^{12}$ hashes).

Working directly in practical units:

If the miner consumes $E$ kWh of energy over some period, the total power-seconds equate to $E \times 1000 \times 3600 = 3.6 \times 10^6 \, E$ Joules.

The total hashes computed are:

$$\text{Total TH} = \frac{\text{Total Joules}}{J} = \frac{3.6 \times 10^6 \, E}{J} \quad [\text{TH}]$$

If we define the hashprice $HP_{\text{per TH}}$ in USD per TH (instead of per PH), the revenue $R$ [USD] from consuming $E$ kWh is simply:
...
$$k = \frac{3.6 \times 10^6}{J} \cdot HP_{\text{per TH}} \quad \text{or equivalently} \quad k = \frac{HP_{\text{daily per PH}}}{J/1000} \cdot \frac{1}{24}$$
...
* Track this metric historically by substituting a time series of hashprice to obtain a historical timeline of revenue per kWh for fixed hardware.

### 3.2 Empirical Regression and Correlation Analysis

Historical data for your mining facility (or a similar one) should include:

* Hourly (or daily) energy consumption $E_t$
* Hourly (or daily) revenue $R_t$
* The corresponding hashprice $HP_t$
...
If you only have the renewable generation profile and not actual mining data, you can compute the **Pearson correlation coefficient** between available power $P_{\text{avail}}(t)$ and the hashprice $HP(t)$ over identical time windows (e.g., daily, hourly). This quantifies whether high-revenue hours tend to coincide with high-availability hours.
...
Define a new variable representing revenue per kWh consumed at time $t$:

$$Y_t = \frac{R_t}{E_t}$$

From the derivation, $Y_t = \frac{3.6 \times 10^6}{J} \cdot HP_t$, which is a simple scaled version of the hashprice. Thus, any model that fits hashprice can be applied to $Y_t$.
